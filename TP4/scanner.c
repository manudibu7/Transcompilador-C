...
const char* token_a_string(Token t) {
    ...
    switch (t.token) {
        case IDENTIFICADOR:
            ...
            break;
        case ENTERO:
            snprintf(buffer, sizeof(buffer), "Entero '%s'", t.lexema);
            break;
        case CONSTANTE:
        ...
        case PORCENTAJE:
            ...
            break;
        case FIN:
            snprintf(buffer, sizeof(buffer), "%s", t.lexema);
            break;
        case ERROR_GENERAL:
            snprintf(buffer, sizeof(buffer), "Error general '%s'", t.lexema);
            break;
        case ERROR_FALTA_IGUAL:
            snprintf(buffer, sizeof(buffer), "Error en asignacion por : solo '%s'", t.lexema);
            break;
        case ERROR_FALTA_DOS_PUNTOS:
            snprintf(buffer, sizeof(buffer), "Error en asignacion por = solo '='");
            break;
        default:
